...
// wifi config
const char* ssid = "3490";
const char* password = "asdf1234567";
...
uint16_t rx_size = 0;
float Bat_Voltage = 0;
// The maximum packet size of data the sx1262 can send is 256 bytes, so im making the data ill send and recieve from the drone smaller(250 byte)
byte rx_buf[250] = {0}, packetBuffer[250] = {0}, DataBuffer[250] = {0}; byte packetLength = 250;


//=================================================================================================
/* function initialises various components and configurations for the program. I starts by setting up the 
EEPROM library and serial communication. Then i configures the WiFi connection and sets up a soft access point. 
After a brief delay, it starts a WiFi server and initialises pin modes for different pins. It blinks an LED to 
indicate status and enables the LoRa module. Finally, it calls the ParaInit() function to initialize the LoRa module with specific parameters.*/
void setup(void)
{
...
/*The purpose of this loop is to continuously receive data from a connected client via WiFi(APM groundcontrol), transmit that 
data using the LoRa module, and also receive LoRa packets and send them back to the client. It establishes 
bidirectional communication between the lora and the connected client, allowing for data exchange over both WiFi and LoRa.*/
void loop (void)
{
...
// read data from wifi client and send to Serial2
while ((size = client.available())) {
           size = (size >= BUFFER_SIZE ? BUFFER_SIZE : size);
           client.read(buff, size);
           SetTxMode();
  LoRa1262.TxPacket(buff, size);
